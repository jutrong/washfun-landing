...
const Cover = ({ onMoveBox, onAccordianMoveBox, onContactMoveBox }: { onMoveBox: () => void, onAccordianMoveBox: () => void, onContactMoveBox: () => void; }) => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      <div className="absolute inset-x-0 w-full h-24 z-10 flex justify-between items-center px-60 lg:px-10">
      ...
      <div className="absolute top-72 left-72 flex flex-col text-white lg:left-1/2 lg:-translate-x-1/2 lg:top-40 w-full">
        <div className="flex flex-col gap-3 mb-16 lg:items-center">
          <p className="text-7xl font-medium  opacity-90 lg:text-6xl">셀프 세차의</p>
          <p className="text-8xl font-bold tracking-wide lg:text-6xl">디지털 혁신</p>
        </div>
        <div className="mb-6 flex  lg:justify-center">
          <p className="text-4xl text-primary lg:text-3xl">워시펀</p>
          <span className="text-4xl text-white lg:text-3xl">과 함께</span>
        </div>
        <div className=" flex lg:justify-center">
          <motion.div
            whileHover={{ scale: 1.05, transition: { duration: 0.5 } }}
            className="flex items-center gap-3 p-3 w-40 h-12 border border-x-2 bg-white rounded-xl z-20 lg:justify-center " onClick={onContactMoveBox}>
            <motion.div
            >
